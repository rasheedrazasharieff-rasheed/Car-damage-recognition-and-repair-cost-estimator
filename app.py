...
def dashboard():
    if request.method == 'POST':
        ...
        # --- NEW SECTION: Fetch User Info for the Report ---
        user_info = {}
        connection = connect_to_db()
        if connection:
            try:
            ...
            # It's good practice to close the connection, though your current app structure handles it loosely
            # connection.close()

        # Fetch part prices using the AGGREGATED counts
        part_prices = get_part_prices(user_email, total_class_counts)

        # --- UPDATED RETURN: Pass user_info to the template ---
